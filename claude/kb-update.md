...
| 1 | nothing to do, or ingest not confirmed | Say WHICH of the three. "Nothing new" means the parts were already in `<root>/incoming/`. **"Download complete. Finish the ingest with..." is the NORMAL result when there is no scheduled task** - that task is optional, the download succeeded, and the run just needs `kb_ingest.py`. Do not present it as a failure or as something the user should have set up. "the task was triggered but wrote no new ingest.log line" means a task does exist, did start, and logged nothing - check `<root>/ingest.log`. |
